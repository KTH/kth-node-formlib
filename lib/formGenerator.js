...
renderOptions
  var outp = registry.getAdapter(fieldValidator, safeGet(() => renderOptions.renderWith) || IInputFieldWidget).render(key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions)
...
module.exports.renderForm = function (options, lang) {
  ...
  var outp = Object.keys(formSchema._fields).map(function (key) {
    ...
    if (onlyFields && onlyFields.indexOf(key) < 0) {
      // If onlyField is set, check that this field is in the list otherwise skip
      return ''
    } else if (excludeFields.indexOf(key) >= 0) {
