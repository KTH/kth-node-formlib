 ...
 * 
 */
module.exports.renderFormAsync = function (options, lang) {
  return module.exports.renderForm(options, lang, true)
}

module.exports.renderForm = function (options, lang, async) {
  ...
  var outp = Object.keys(formSchema._fields).map(function (key) {
    ...
    if (onlyFields && onlyFields[key] === undefined) {
      ...
      return ''
    } else if (typeof customWidgets[key] === 'function') {
      // Check if a custom widget has been provided, in which case call it
      return customWidgets[key]({
        data: data, 
        key: key, 
        objectNamespace: objectNamespace,
        schema: formSchema, 
        errors: options.errors, // The full error object
        disabled: disabled,
      ...
      const opts = {
        ...
        submitted: options.submitted,
        renderOptions: renderOptions,
        customWidgets: customWidgets[key] 
      }
      if (async) {
        return wrapper.renderAsync(opts)
      } else {
        return wrapper.render(opts)
      }
    }
  })
