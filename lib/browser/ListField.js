...
var doToggleActionBtns = function (containerEl) {
  var $containerEl = $(containerEl)
  var minItems = parseInt($(containerEl).attr('data-minItems'))
  var maxItems = parseInt($(containerEl).attr('data-maxItems'))
  var nrofItems = $(containerEl).children('.IListField-RowTemplate').toArray().length

  if (nrofItems <= minItems || nrofItems <= 0) {
    // Hide remove button and show add button
    // $('.IListField-AddBtn').animate({ width: '100%', "padding-left": "1.8em", opacity: 1}, {duration: 300, done: function () { $(this).removeClass('ActionBtnHidden') }})
    $('.IListField-AddBtn').removeClass('ActionBtnHidden')
    // $('.IListField-RemoveBtn').animate({ width: 0, "padding-left": 0, opacity: 0}, { duration: 300, done: function () { $(this).addClass('ActionBtnHidden') } })
    $('.IListField-RemoveBtn').addClass('ActionBtnHidden')
  } else if (nrofItems >= maxItems) {
    // Show remove button and hide add button
    // $('.IListField-RemoveBtn').animate({ width: '100%', "padding-left": "1.8em", opacity: 1}, {duration: 300, done: function () { $(this).removeClass('ActionBtnHidden') }})
    $('.IListField-RemoveBtn').removeClass('ActionBtnHidden')
    // $('.IListField-AddBtn').animate({ width: 0, "padding-left": 0, opacity: 0}, { duration: 300, done: function () { $(this).addClass('ActionBtnHidden') } })
    $('.IListField-AddBtn').addClass('ActionBtnHidden')
  }
}
