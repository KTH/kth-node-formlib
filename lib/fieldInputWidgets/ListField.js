...
var ListInputAdapter = createAdapter({
  ...
  render: function render(key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions, customWidgets) {
    ...
    var outp = '<div class="' + typeName + '-Container" data-id-template="' + newObjectNamespace + '[{index}]" data-max-items="' + (this.context._maxItems || '') + '" data-min-items="' + (this.context._minItems || '') + '">'
    ...
    data && data.forEach((dataItem, index) => {
      ...
      outp += '<div class="' + typeName + '-Row' + (typeName !== 'IListField' ? ' IListFieldRow' : '') + '">'
    ...
    if (!this.context.readOnly) {
      ...
      outp += '<a class="' + classnames(removeBtnCls) + '" role="button" data-row-class="' + typeName + '-Row">' + (i18n ? i18n.message('field_action_remove_selected', lang) : 'Remove selected') + '</a>'
      outp += '<button class="' + classnames(addBtnCls) + '" data-row-class="' + typeName + '-Row">'
