  ...
  render: function render(key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions, customWidgets) {
    ...
    if (!this.context.readOnly) {
      ...
      var removeBtnCls = { 
        ...
        ActionBtnHidden: (this.context.minItems ? this.context.minItems >= (data && data.length || 0) : (data && data.length || 0) <= 0)
      }
      ...
      var addBtnCls = { 
        btn: true,
        // Only show remove button if we have more than zero items and more than minItems
        ActionBtnHidden: (this.context.maxItems ? (this.context.maxItems || 0) >= (data && data.length || 0) : false)
      }
