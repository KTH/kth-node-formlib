...
var ListInputAdapter = createAdapter({
  ...
  render: function render(key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions, customWidgets) {
    ...
    var outp = '<div class="' + typeName + '-Container" data-id-template="' + newObjectNamespace + '[{index}]" data-maxItems="' + (this.context.maxItems || '') + '" data-minItems="' + (this.context.minItems || '') + '">'
    ...
    if (!this.context.readOnly) {
      ...
      var addBtnCls = { 
        btn: true,
        // Only show remove button if we have more than zero items and more than minItems
        ActionBtnHidden: (this.context.maxItems ? this.context.maxItems >= (data && data.length || 0) : (data && data.length || 0) > 0)
      }
