...
var ListInputAdapter = createAdapter({
  ...
  render: function render(key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions, customWidgets) {
    ...
    var outp = []
    outp.push('<div class="' + typeName + '-Container" data-id-template="' + newObjectNamespace + '[{index}]" data-max-items="' + (this.context._maxItems || '') + '" data-min-items="' + (this.context._minItems || '') + '">')
    ...
    outp.push('<div class="' + typeName + '-RowTemplate">')
    ...
    data && data.forEach((dataItem, index) => {
      ...
      outp.push('<div class="' + typeName + '-Row' + (typeName !== 'IListField' ? ' IListFieldRow' : '') + '">')
    ...
    if (!this.context.readOnly) {
      ...
      var removeBtnCls = { 
        ...
        ActionBtnHidden: (this.context._minItems ? this.context._minItems >= (data && data.length || 0) : (data && data.length || 0) <= 0)
      }
      removeBtnCls[typeName + '-RemoveBtn'] = true
      if (typeName !== 'IListField') {
        removeBtnCls['IListField-RemoveBtn'] = true
      }

      var addBtnCls = { 
        btn: true,
        // Only show remove button if we have more than zero items and more than minItems
        ActionBtnHidden: (this.context._maxItems ? (this.context._maxItems || 0) <= (data && data.length || 0) : false)
      }
      addBtnCls[typeName + '-AddBtn'] = true
      if (typeName !== 'IListField') {
        addBtnCls['IListField-AddBtn'] = true
      }

      outp.push('<div class="' + typeName + '-ActionButtons">')
      outp.push('<a class="' + classnames(removeBtnCls) + '" role="button" data-row-class="' + typeName + '-Row">' + (i18n ? i18n.message('field_action_remove_selected', lang) : 'Remove selected') + '</a>')
      outp.push('<button class="' + classnames(addBtnCls) + '" data-row-class="' + typeName + '-Row">')
    ...
    outp.push('<div class="' + typeName + '-ActionButtonsClearfix""></div>')
    outp.push('</div>')
...
var ListDisplayAdapter = createAdapter({
  ...
  render: function (key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions, customWidgets) {
    ...
    var outp = []
    outp.push('<div class="' + typeName + '-Container" data-id-template="' + newObjectNamespace + '[{index}]">')
    ...
    data && data.forEach((dataItem, index) => {
      const rowObjectNamespace = newObjectNamespace + '[' + index + ']'
      outp.push('<div class="' + typeName + '-Row">')
