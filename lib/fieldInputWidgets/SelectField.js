...
function _getOptions (fieldValidator, async) {
  if (!Array.isArray(fieldValidator.options) && fieldValidator.options.utilityInterface) {
    try {
      var util = registry.getUtility(fieldValidator.options.utilityInterface, fieldValidator.options.name)
    } catch (err) {
      throw new Error("SelectField can't find utility (" + fieldValidator.options.utilityInterface.name + ", " + fieldValidator.options.name + ")")
    }
...
var SelectInputAdapter = createAdapter({
  ...
  renderAsync: function (key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions) {
    ...
    return _getOptions(this.context, true)
      .then((options) => {
  ...
  render: function (key, data, fieldError, lang, objectNamespace, disabled, submitted, renderOptions) {
    const async = safeGet(() => renderOptions._async)
    const options = safeGet(() => renderOptions._options) || _getOptions(this.context, async)
